...
uint32_t expr(char *e, bool *success)
{
  ...
  *success = true;
  ...
  {
    if (tokens[i].type == TK_MUL && ((i == 0) || (tokens[i - 1].type != TK_NUMBER && tokens[i - 1].type != TK_HEX && tokens[i - 1].type != TK_RB && tokens[i - 1].type != TK_REGISTER)))
    { //指针
      tokens[i].type = TK_POINT;
      tokens[i].level = 6;
    }
    if (tokens[i].type == TK_SUB && ((i == 0) || (tokens[i - 1].type != TK_NUMBER && tokens[i - 1].type != TK_HEX && tokens[i - 1].type != TK_RB && tokens[i - 1].type != TK_REGISTER)))
    { //负号
